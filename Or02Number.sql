...
--금액일때 100원단위 이하 절삭과 같은 형태로 사용함.
select trunc(12345.123456, -2) from dual;

/*
시나리오] 사월테이블에서 영업사원이 1000불에 대한 커미션을 계산하여
    급여에 합한 결과를 출력하는 쿼리문을 작성하세요.
    Ex) 급여:1000, 보너스율:0.1
        => 1000 + (1000*0.1) = 1100
*/
--1.영업사원을 먼저 찾는다.(영업사원은 job_id가 SA_xxx와 같이 시작한다.)
...
/*
연습문제] 사원테이블에서 보너스율이 있는 사원만 인출한 후 보너스율을 소수점
    1자리로 표한시오.
    출력내용 : 이름, 급여, 보너스율
*/
---커미션이 있는 사원만 인출
...
--첫번째항목 : 소수이하 6자리까지 표현하므로 7을 올림처리하여 0.123457
--두번째항목 : 소수이하 4자리까지 표현하므로 1을 버림처리하여 2.3456
